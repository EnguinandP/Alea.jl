...
Definition genLeafTree (chosen_ctor : LeafCtorTree) (stack1 : nat) (stack2 : nat) (stack3 : nat) : G (Tree) :=
  match chosen_ctor with
  | LeafCtorTree_Leaf => 
    (returnGen (Leaf ))
  end.

Fixpoint genTree (size : nat) (chosen_ctor : CtorTree) (stack1 : nat) (stack2 : nat) (stack3 : nat) : G (Tree) :=
  match size with
  | O  => match chosen_ctor with
    ...
    | CtorTree_Node => 
      ...
      (fun param_variantis => (let '(MkLeafCtorTreeLeafCtorTree ctor1 ctor2) := param_variantis in
        ...
        (bindGen 
        (* GenNat1 *)
        (let _weight_1 := match (stack1, stack2, stack3) with
        | (0, 0, 0) => 50
        | _ => 500
        end
        in
        ...
        (let _weight_2 := match (stack1, stack2, stack3) with
        | (0, 0, 0) => 50
        | _ => 500
        end
        in
        ...
        (let _weight_4 := match (stack1, stack2, stack3) with
        | (0, 0, 0) => 50
        | _ => 500
        end
        in
        ...
        (fun p1 => 
          (bindGen (genLeafTree ctor1 stack2 stack3 1) 
          (fun p2 => 
            (bindGen (genLeafTree ctor2 stack2 stack3 3) 
            (fun p3 => 
  ...
  | S size1 => match chosen_ctor with
    ...
    | CtorTree_Node => 
      (bindGen 
      (* Frequency3 *) (freq [
        (* 1 *) (match (size, stack1, stack2, stack3) with
        (* This match is dead code *) | _ => 500
        end,
        (returnGen (MkCtorTreeCtorTree CtorTree_Leaf CtorTree_Leaf))); 
        (* 2 *) (match (size, stack1, stack2, stack3) with
        (* This match is dead code *) | _ => 500
        end,
        (returnGen (MkCtorTreeCtorTree CtorTree_Node CtorTree_Leaf))); 
        (* 3 *) (match (size, stack1, stack2, stack3) with
        (* This match is dead code *) | _ => 500
        end,
        (returnGen (MkCtorTreeCtorTree CtorTree_Leaf CtorTree_Node))); 
        (* 4 *) (match (size, stack1, stack2, stack3) with
        (* This match is dead code *) | _ => 500
        end,
      ...
      (fun param_variantis => (let '(MkCtorTreeCtorTree ctor1 ctor2) := param_variantis in
        ...
        (bindGen 
        (* GenNat2 *)
        (let _weight_1 := match (size, stack1, stack2, stack3) with
        (* This match is dead code *) | _ => 500
        end
        in
        bindGen (freq [
        ...
        (let _weight_2 := match (size, stack1, stack2, stack3) with
        (* This match is dead code *) | _ => 500
        end
        in
        bindGen (freq [
        ...
        (let _weight_4 := match (size, stack1, stack2, stack3) with
        (* This match is dead code *) | _ => 500
        end
        in
        bindGen (freq [
        ...
        (fun p1 => 
          (bindGen (genTree size1 ctor1 stack2 stack3 2) 
          (fun p2 => 
            (bindGen (genTree size1 ctor2 stack2 stack3 4) 
            (fun p3 => 
              (returnGen (Node p1 p2 p3)))))))))))
    end
  end.
...
Definition gSized :=
  ...
  (fun init_ctor => (genTree 0 init_ctor 0 0 0))).
