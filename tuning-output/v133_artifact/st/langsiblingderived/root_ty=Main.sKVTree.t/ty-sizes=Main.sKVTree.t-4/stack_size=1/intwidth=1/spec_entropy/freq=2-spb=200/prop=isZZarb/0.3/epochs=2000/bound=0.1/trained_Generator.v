...
Fixpoint is_zigzag_arb_aux (t : Tree) (has_switched_to_left : bool) : bool :=
  match t with
  | E => true
  | T _ l r =>
    match l, r with
    | E, T _ _ _ =>  (* right step *)
      if has_switched_to_left then false
      else is_zigzag_arb_aux r false
    | T _ _ _, E =>  (* left step *)
      is_zigzag_arb_aux l true
    | E, E =>  (* both empty - leaf node *)
      true
    | T _ _ _, T _ _ _ =>  (* both T - invalid spine *)
      false
    end
  end.

Definition is_zigzag_arb (t : Tree) : bool :=
  is_zigzag_arb_aux t false.
...
QuickChickWith (updMaxSuccess stdArgs numRuns) (count_sticky is_zigzag_arb ).
(* QuickChickWith (updMaxSuccess stdArgs numRuns) (count_sticky is_sticky ).
